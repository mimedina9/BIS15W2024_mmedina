...
```{r}
fish %>% 
  mutate(length_mm = length*10) %>%  #made a conversion 
...
mammals %>% 
  select(genus, species, newborn) %>%
  mutate(newborn_new = ifelse(newborn == -999.00, NA, newborn))%>% #create new variable, look into variable and put NA for -999 but if it is anything else it will leave it alone 
  arrange(newborn)
```
