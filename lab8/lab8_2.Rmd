...
```{r}
life_history_no_nas <- 
  read_csv(file = "data/mammal_lifehistories_v3.csv", na = c("NA", " ", ".", "-999")) %>% clean_names() # creating a new object/ dataframe that doesn't have any NA's
```

Recheck
```{r}
life_history_no_nas %>% map_df(~ sum(is.na(.))) # fixed most of the problems
```
...
```{r}
hist(life_history_no_nas$max_life) #found another way that NA's are represented
```
