...
takes one or the other variable not both
```{r}
filter(fish, lakeid == "AL" | length > 350)
```
...
```{r}
mammals_mgn <- select(mammals, "genus", "order", "mass", "gestation", "newborn")
mammals_mgn
```

```{r}
mammals_mgn2 <-filter(mammals_mgn, order == "Primates")
mammals_mgn2
```

```{r}
mammals_mgn3 <- filter(mammals_mgn2, genus %in% c("Lophocebus", "Erythrocebus", "Macaca"))
mammals_mgn3
```
