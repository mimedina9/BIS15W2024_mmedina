...
Load the data.  
```{r}
mammals_lh <-read_csv("data/mammal_lifehistories_v2.csv")
```
...
```{r}
mammals_new <-rename(mammals_lh, afr="AFR", genus="Genus", wean_mass="wean mass", max_life="max. life", litter_size="litter size", litters_per_year="litters/year")
```
...
```{r}
select_if(mammals_new, ~!is.numeric(.))
```
