...
glimpse(deserts)
```
The data are tidy and NAs are empty cells. 

2. How many genera and species are represented in the data? What are the total number of observations? Which species is most/ least frequently sampled in the study?
```{r}
str(deserts)
```
The total number of observations is 34,786. 
```{r}
deserts %>% 
  count(genus, sort=T)
```
...
deserts %>% 
  ...
  arrange(n_samples)
```
The species most frequently studied is marriami and the species the least studied are the clarki, scutalatus, tereticaudus, tigris, uniparens, and viridis. 
