...
```{r}
p +geom_point(size=1.25) #adjust the size of the dots in the plot
```
...
homerange %>% 
  ...
  scale_y_continuous(labels = scales::percent)+ #turns it into a percentage
  coord_flip()
```
...
homerange %>% 
  ggplot(aes(x = class, y = log10.mass, group = taxon, fill=taxon)) +
  geom_boxplot(alpha=0.4) #add aesthetics, making the colors a little lighter from before
```
