...
deserts %>% 
  ...
  mutate(year=as.factor(year)) %>% #makes it so the year numbers are not treated as if they were to do math
...
deserts %>% 
  ...
  geom_line()+
  geom_point(shape=2)+ #you can experiment with shapes; this is layering another graph 
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "black", fill = "orchid2", bins=10)+ #color is the outline of the bars and the fill is filling in the bars
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(color="black", fill="plum", alpha=0.3)+ #we can adjust the number of bins with the bins argument
...
homerange %>% 
  ggplot(aes(x = log10.hra)) +
  geom_density(color="black", fill="plum", alpha=0.3)+ #we can adjust the number of bins with the bins argument
  labs(title = "Distribution of Homerange")
...
homerange %>% 
  ggplot(aes(x = log10.hra)) +
  geom_histogram(aes(y=after_stat(density)), color="grey", fill="plum", alpha=0.3)+ #we can adjust the number of bins with the bins argument
...
homerange %>% 
  mutate(mass_category=case_when(log10.mass<=1.75 ~ "small", 
                                 log10.mass>1.75 & log10.mass<=2.71 ~ "medium", 
...
```{r}
homerange %>% 
  mutate(range_category=case_when(log10.hra<3.5 ~ "very small", 
                                  log10.hra>=3.65 & log10.hra<=4.59 ~ "small", 
                                  log10.hra>4.59 & log10.hra<=4.59 ~ "medium", 
