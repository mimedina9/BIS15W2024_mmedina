...
There are 83 rows and 11 variables, this can be seen in the environment when you create the data frame sleep. it shows 83 rows and 11 columns(variables). 
...
```{r}
sapply(sleep, function(sleep) sum(is.na(sleep)))
```
...
```{r}
sleep <- filter(msleep, bodywt>=200)
sleep
```

```{r}
large_mammals <- data.frame(filter(msleep, bodywt>=200))
large_mammals
```


```{r}
sleep <- filter(msleep, bodywt<=19)
sleep
```

```{r}
small_mammals <- data.frame(filter(msleep, bodywt<=19))
small_mammals
```
